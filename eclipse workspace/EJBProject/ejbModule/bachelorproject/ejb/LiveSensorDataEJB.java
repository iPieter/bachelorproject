 ...
 * LiveSensorData instances. It validates new objects and handles errors when,
 * for example, no entries in the database exist.
 * 
...
public class LiveSensorDataEJB
{
	 ...
	 * 
	 * @param lsd
	 *            The LiveSensorDataObject to be persisted
	 */
	...
	public void closeLiveSensorData( int lsdID )
	{
		...
		if ( sensordata != null )
		{
			...
			for ( LiveSensorDataEntry lsde : sensordata.getEntries() )
			{
				...
				if ( lsde.getYaw() < minYaw ) minYaw = lsde.getYaw();
				if ( lsde.getYaw() > maxYaw ) maxYaw = lsde.getYaw();
				if ( lsde.getRoll() < minRoll ) minRoll = lsde.getRoll();
				if ( lsde.getRoll() > maxRoll ) maxRoll = lsde.getRoll();
			}

			SimpleDateFormat format = new SimpleDateFormat( "yyyy_MM_dd_HH_mm_ss" );
			String fileName = sensordata.getTraincoach().getType() + "_" + sensordata.getTraincoach().getName() + "_"
					+ format.format( sensordata.getDate() ) + ".json";
