 ...
 * <p>
 * In order to minimize loading times, we have opted to load these images via
 * AJAX. This REST endpoint allows authenticated users to request these files.
 * 
...
public class IssueAssetRestService
{
	...
	public Response getIssueAsset( @PathParam( "id" ) int id )
	{
		...
		if ( asset == null || asset.getLocation().equals( "" ) ) return Response.status( Status.NOT_FOUND ).build();
	 ...
	 * 
	 * @param id
	 *            The ID of the issue asset to fetch
	 */
	@GET
	@Path( "data/{id}" )
	@Produces( "text/json" )
	 ...
	 * 
	 * @param id
	 *            The ID of the issue who's assets to fetch
	 */
	@GET
	...
	public Response getIssueAssetDataByIssueID( @PathParam( "id" ) int id )
	{
		...
		if ( issue == null ) return Response.status( Status.NOT_FOUND ).build();
	...
	public Response getAllAssets()
	{
		...
		if ( issues == null ) return Response.status( Status.NOT_FOUND ).build();
	...
	public Response addIssueAsset( MultipartFormDataInput input )
	{
		...
		if ( inputParts == null ) return Response.status( Status.BAD_REQUEST ).build();
		...
		for ( InputPart inputPart : inputParts )
		{
			try
			{
				...
				if ( issue != null && issue.getMechanic().getId() == userID )
				{
					...
					String path = System.getProperty( "user.home" ) + "/project_televic/issue_assets/"
							+ format.format( now ) + "_" + userID + ".png";
					...
					if ( hasFile ) asset.setLocation( path );
					else asset.setLocation( "" );
