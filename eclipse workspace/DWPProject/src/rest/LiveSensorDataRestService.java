...
public class LiveSensorDataRestService
{
	...
	public Response addEntry( @PathParam( "lsdID" ) int lsdID, @PathParam( "lat" ) double lat,
			@PathParam( "lng" ) double lng, @PathParam( "speed" ) double speed, @PathParam( "accel" ) double accel,
			@PathParam( "yaw" ) double yaw, @PathParam( "roll" ) double roll )
	{
		...
		if ( !lsdeEJB.addEntry( lsdID, lsde ) ) return Response.status( Status.BAD_REQUEST ).build();
	 ...
	 * 
	 * @param lsdID
	 *            The ID of the associated LiveSensorData object
	 * @param from
	 *            The data from which to retrieve the entries
	 */
	@GET
	@Path( "get/{lsdID}/{date}" )
	@Produces( "text/json" )
	...
	public Response getSensorDataEntries( @PathParam( "lsdID" ) int lsdID, @PathParam( "date" ) String from )
	{
		...
		for ( LiveSensorDataEntry lsde : entries )
		{
			...
			if ( i != entries.size() - 1 ) out += "}," + newLine;
			else out += "}" + newLine;
			i++;
		}
		out += "]" + newLine;
	 ...
	 * 
	 * @param lsdID
	 *            The {@link LiveSensorData} to be closed.
	 */
	@GET
	@Path( "/stop/{lsdID}" )
