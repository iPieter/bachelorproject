...
public class UserAssetRestService
{
	...
	/**
	 * Returns a image when provided with the correct resource name, likely
	 * provided by the caller of the REST service. If the resource name ables
	 * the function to find a file (.png) with the same name, it is returned.
	 * 
	 * @author Pieter Delobelle
	 * @version 1.0.0
	 * @param resource
	 *            The resource name that's provided by the function caller.
		...
		for ( InputPart inputPart : inputParts3 )
		{
			try
			{
				...
				if ( user != null )
				{
					...
					// The reason I don't use id's, is because this makes them
					// non-guessable, as
					// opposed to id's. Changes for collisions are 64^8 = 218 *
					// 10^12

					String identifier = Base64.getEncoder().encodeToString( UserService.salt( 10 ) ).substring( 0, 8 );

					String path = System.getProperty( "user.home" ) + "/project_televic/user_assets/" + identifier
							+ ".png";
