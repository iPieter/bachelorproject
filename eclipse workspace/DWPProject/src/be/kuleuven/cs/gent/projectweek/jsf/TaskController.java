...
public class TaskController implements Serializable{
	...
	public String doCreateTask()
	{
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Task created",
					"The task" + task.getDescription() + " has been created with id=" + task.getId()));
			return "viewTasks.xhtml?faces-redirect=true";

		} catch (TaskValidationException e) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Failed to create task.",
					"Please enter correct/complete task details."));
			return null;
		}
	...
	public String doFileTaskReport()
	{
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Task report filed",
					"The task report has been succesfully filed"));
			return "viewOpenTasks.xhtml?faces-redirect=true";

		} catch (TaskValidationException e) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Failed to file task report.",
					"Please enter correct/complete task details."));
			return null;
		}
